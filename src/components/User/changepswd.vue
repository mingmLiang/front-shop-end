    ...
    <section class="wapper">
     <Form ref="formInline" :model="formInline" :rules="ruleInline"  inline style="text-align: center;">
        <FormItem prop="user"  label="手机号">
            <Input v-model="formInline.phone" type="text" placeholder="请输入手机号"></Input>
        </FormItem><br>
<!--          <FormItem prop="code">
          <Row>
              <Col span="14">
         ...
         <FormItem prop="oldpassword"  label="原始密码">
            <Input type="password" v-model="formInline.oldpassword" placeholder="请输入原始密码"></Input>
        </FormItem><br>
        ...
        <FormItem  style="width:80%;">
            <v-bt1 @click.native="handleSubmit('formInline')"><span slot="title">修改</span></v-bt1>
        </FormItem>
    </Form>
    </section>
  </div>
...
import  { api } from '@/global/siteConfiguration';
 ...
 export default {
   ...
   data () {
            ...
            handleSubmit(name) {
              let this_=this;
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        this.$http.post(api.changePwd, {'phone':this.formInline.phone,'password':this.formInline.password,'oldpassword':this.formInline.oldpassword}, function(data){
                          if(data.code===400){
    ...
    mounted(){
      this.formInline.phone=sessionStorage.getItem("qtt_user_phone_num");
    }
 }
